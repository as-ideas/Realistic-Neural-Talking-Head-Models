...
class Generator(nn.Module):
    P_LEN = 2 * (
        512 * 2 * 5 + 512 * 2 + 512 * 2 + 512 + 256 + 256 + 128 + 128 + 64 + 64 + 3
    )
    slice_idx = [
        0,
        512 * 4,  # res1
    ...
    def __init__(self, in_height, finetuning=False, e_finetuning=None):
        ...
        self.resDown1 = ResBlockDown(
            3, 64, conv_size=9, padding_size=4
        )  # out 64*128*128
    ...
    def forward(self, x, y, i):
        ...
        if self.finetuning:
            out = (
                torch.bmm(
                    out.transpose(1, 2),
                    (self.w_prime.unsqueeze(0).expand(out.shape[0], 512, 1)),
                )
                + self.b
            )
        else:
...
class Cropped_VGG19(nn.Module):
    ...
    def forward(self, x):
        ...
        pool1 = F.max_pool2d(
            pool1_pad, kernel_size=(2, 2), stride=(2, 2), padding=0, ceil_mode=False
        )
        ...
        pool2 = F.max_pool2d(
            pool2_pad, kernel_size=(2, 2), stride=(2, 2), padding=0, ceil_mode=False
        )
        ...
        pool3 = F.max_pool2d(
            pool3_pad, kernel_size=(2, 2), stride=(2, 2), padding=0, ceil_mode=False
        )
        ...
        pool4 = F.max_pool2d(
            pool4_pad, kernel_size=(2, 2), stride=(2, 2), padding=0, ceil_mode=False
        )
...
class PartialInceptionNetwork(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        """
        x = torch.nn.functional.interpolate(x, (299, 299))
        assert x.shape[1:] == (
            3,
            299,
            299,
