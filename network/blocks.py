...
class ResBlockDown(nn.Module):
    def __init__(self, in_channel, out_channel, conv_size=3, padding_size=1):
        ...
        # right
        self.conv_r1 = nn.utils.spectral_norm(
            nn.Conv2d(in_channel, out_channel, conv_size, padding=padding_size)
        )
        self.conv_r2 = nn.utils.spectral_norm(
...
class ResBlock(nn.Module):
    ...
    def forward(self, x, psi_slice):
        ...
        out = self.conv2(out)
        out = adaIN(
            out, psi_slice[:, C // 2 : 3 * C // 4, :], psi_slice[:, 3 * C // 4 : C, :]
        )
...
class ResBlockUp(nn.Module):
    ...
    def forward(self, x, psi_slice):
        ...
        mean2 = psi_slice[
            :, 2 * self.in_channel : 2 * self.in_channel + self.out_channel, :
        ]
        std2 = psi_slice[
            :,
            2 * self.in_channel
            + self.out_channel : 2 * (self.in_channel + self.out_channel),
            :,
        ]
